...
review_create_response=$(curl -s -X POST "${BASE_URL}/api/v1/reviews" \
    ...
    -d "{\"user_id\": \"${regular_user_id}\", \"place_id\": \"${place2_id}\", \"rating\": 5, \"text\": \"Fantastic stay!\"}")
...
place_reviews=$(curl -s -X GET "${BASE_URL}/api/v1/places/${place2_id}/reviews")
print_response "Public Get Reviews for Place (City Apartment)" "$place_reviews"

# Cleanup (Optional) - Delete the new place (Admin only)
delete_response=$(curl -s -X DELETE "${BASE_URL}/api/v1/places/${place_id}" \
    -H "Authorization: Bearer ${admin_token}")
print_response "Admin Deletes New Place" "$delete_response"
